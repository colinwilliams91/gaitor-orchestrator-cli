...
import { HARNESS_IDS, type HarnessId } from './prompts.js';
...
const BASE_TEMPLATE_ENTRIES: FeatureMapping[] = [
...
const HARNESS_FILE_MAP: Record<HarnessId, FeatureMapping[]> = {
  copilot: [
...
export async function scaffold({ projectName, targetDir, features }: ScaffoldOptions): Promise<void> {
  ...
  const harnessEntries = new Map<string, FeatureMapping>();
